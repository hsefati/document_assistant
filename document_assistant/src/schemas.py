...
class AnswerResponse(BaseModel):
    ...
    sources: List[str] = Field(
        default_factory=list, description="List of source document IDs used"
    )
...
class UserIntent(BaseModel):
    """User intent classification for routing and processing"""

    intent_type: Literal[
        "qa", "summarization", "calculation", "unknown"
    ] = Field(description="The classified category of the user's request")
    confidence: float = Field(
        ge=0, le=1, description="Confidence score of the intent classification"
    )
    reasoning: str = Field(
