...
def create_calculator_tool(logger: ToolLogger):
    ...
    def calculator(expression: str) -> str:
        ...
        Args:
            expression: A mathematical expression to evaluate (e.g., "10 + 5", "100 * 2.5")

        Returns:
            The result of the calculation or an error message

        Allowed operations: +, -, *, /, //, %, ** (addition, subtraction, multiplication, division, floor division, modulo, exponentiation)
        """
        try:
            # Validate expression for safety - only allow basic math operations and numbers
            allowed_chars = set("0123456789+-*/%().** ")
            if not all(c in allowed_chars for c in expression):
                error_msg = "Invalid characters in expression. Only numbers and operators (+, -, *, /, %, **) are allowed."
                logger.log_tool_use(
            ...
            dangerous_patterns = ["__", "import", "exec", "eval", "lambda"]
            if any(pattern in expression.lower() for pattern in dangerous_patterns):
                error_msg = "Expression contains forbidden patterns."
                logger.log_tool_use(
                    "calculator", {"expression": expression}, {"error": error_msg}
                )
                return error_msg
